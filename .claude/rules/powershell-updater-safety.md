...
- Remember a Winget `1605` outcome only after a bounded exact-ID inventory query confirms absence. Invalidate that memory if the exact query later disagrees, and never suppress the recovery choices when durable persistence fails.
