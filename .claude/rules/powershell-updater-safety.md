...
- Defender `hr=0x8007007F` (`ERROR_PROC_NOT_FOUND`) is a broken Defender platform install, not a content or network condition: it fails identically on every same-boot invocation. Classify it as a terminal failure with a platform-repair attention detail instead of queueing retries. Ordinary signature-update failures stay retryable.
