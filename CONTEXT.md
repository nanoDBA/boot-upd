...
**Deferred** — work deliberately not attempted or not attributable to this run, recorded as outstanding inventory. Deferred work is never counted as a verified update and never treated as retry fuel. It does not make a phase fail, but it does prevent an unqualified claim of convergence.

**Quarantine** — a reversible, durable block placed on a single package so it stops being attempted. Distinct from deferred: quarantine is an action the updater takes against the package manager, and it persists until explicitly lifted.
