...
On a new Windows checkout, run `./tools/Initialize-BeadsCredential.ps1` once. It reuses an existing named credential when available; otherwise it stores the central Dolt username and password in a DPAPI-protected Windows Credential Manager generic credential. The password is prompted twice without echo; use `-Replace` only when intentionally rotating or migrating the credential. If BetterCredentials is missing, the helper installs it for the current user with `-AllowClobber`.
