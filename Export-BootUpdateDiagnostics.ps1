...
$logs = @(Get-ChildItem -LiteralPath $SourceDirectory -File -ErrorAction Stop |
    Where-Object { $_.Name -match '^BootUpdateCycle(?:\.(?:providers|aws))?(?:\.\d{8}-\d{6})?\.log$' -or $_.Name -in @('BootUpdateCycle-repair-plan.txt','BootUpdateCycle-winget-quarantine.json') } |
